...
const Booking = () => {
  ...
  const UPI_AMOUNT = card.price.replace(/[₹\s]/g, "");
  const cleanTitle = card.title.replace(/\/+$/, "");
  const UPI_NOTE = encodeURIComponent(cleanTitle);

  const upiLink = `upi://pay?pa=${UPI_VPA}&pn=${encodeURIComponent(
    UPI_NAME
  )}&am=${UPI_AMOUNT}&cu=INR&tn=${UPI_NOTE}`;

  return (
    <div className="text-white" style={{ backgroundImage: `url(${img})` }}>
      <h1 className="font-bold text-center text-3xl py-12 px-6">
